...
## 프로젝트 정보
- **게임 타입**: 뱀서라이크 환경정화 시뮬레이터 (3클래스 + 무기/패시브 시스템)
- **엔진**: Phaser.js 3.80.1
...
| 클래스 | 아이콘 | 특성 | 시작 무기 |
|--------|--------|------|----------|
| 준설공 | 🪣 | HP +20%, 물 공격 +30% | 준설호스 |
| 세정공 | 🔫 | 범위 +25%, 쿨다운 -15% | 정화 필드 |
| 신호수 | 🚩 | 크리티컬 +10%, 이동속도 +15% | 오염측정기 |
...
- 2026-02-08: UI/UX 대규모 개선 + 리롤 1회 제한
  ...
  - 왼쪽 HUD: VS스타일 2열 슬롯 그리드 리뉴얼 (빈슬롯 표시, 레벨 도트, 48px 슬림 패널)
  - 무기 설명 12종 전체 리라이트 (행동 중심 문장으로 변경)
  - 패시브 설명 16종 전체 리라이트 (효과 명확히 표기)
  - 레벨업 카드: 아이콘 40px 확대, Lv.N→N+1 형식, 무기/패시브 타입 구분, 보너스 표시
  - 레벨업 리롤: 무제한→1회 제한
- 2026-02-08: 종합 밸런스 조정 + 조이스틱 개선
  - 조이스틱 터치 영역: 왼쪽 절반 → 전체 화면 (어디서든 터치 이동 가능)
  - 준설공 시작 무기: waterGun(고압세척기) → dredgeHose(준설호스)로 변경
  - 준설호스 너프: baseDamage 8→5, baseCooldown 100→300, range 300→200, angle 60→50
  - 무기 스케일링 너프: lv*0.2→lv*0.12, lv*0.25→lv*0.15 (전 무기 적용)
  - 패시브 너프: damage 0.1→0.06, speed 0.08→0.05, maxHp 25→15, magnet 0.3→0.25, cooldown 0.05→0.03, area 0.1→0.07, critChance 0.03→0.02, critDamage 0.15→0.10, lifesteal 0.01→0.005
  - 적 너프: 전 8종 HP/데미지 하향 (sludge hp15→12 dmg8→5, waste hp45→35 dmg12→8, sludgeGiant hp130→100 dmg25→18 등)
  - 뱀파이어 서바이버스 밸런스 리서치 기반 조정
- 2026-02-07: 프로시저럴 사운드 시스템 추가 (10종)
  - GameSoundManager 클래스 (Web Audio API, 프로시저럴 생성)
  - 사격/적피격/적처치/플레이어피격/경험치/레벨업/화염탄/충격파/낙뢰/보스경고
  - 동시 재생 최대 5개, 같은 사운드 최소 간격 50~100ms
  - 🔊/🔇 토글 버튼 (타이틀 + 게임 HUD), localStorage 저장
  - 모바일 AudioContext resume() 해금
  - 추가 파일 용량 0 (코드 생성 사운드)
- 2026-02-07: 소환 무기 전면 교체 (프리즈 근본 해결)
  - 안전콘(터렛 소환) → 화염탄(캐릭터 직접 폭발탄 발사, A방식)
  - 청소차(탱크 소환) → 충격파(캐릭터 중심 원형 폭발+넉백, C방식)
  - 환경드론(드론 소환) → 공습(적 다수 낙뢰 공격, C방식)
  - 드론 밸런스 조정: baseDamage 6→25, baseCooldown 500→3000ms
  - 기존 소환 코드는 블록 주석(/* */)으로 보존
  - 소환 엔티티(컨테이너, 타이머바, 범위원) 완전 제거 → 오브젝트 수 95% 감소
- 2026-02-07: 소환 무기 성능 최적화 (게임 프리즈 해결)
  - fireCone/fireTruck/fireDrone 타이머 50→100ms (연산량 50% 절감)
  - Math.sqrt() → 거리제곱 비교 (적 탐색/폭발 범위 판정)
  - 미사일 트레일: 7~8개→3개, 간격 20~30ms→60ms
  - 폭발 이펙트: 3~4레이어→2레이어
  - 퇴장 파티클: 절반으로 축소
  - 탱크 카메라 쉐이크 제거
  - 동시 소환 최대 2개 제한 (activeSummons 카운터)
- 2026-02-07: 준설공 아이콘 🪣→🚿 변경
- 2026-02-07: 갤러그 게임 제거, 원본 뱀서라이크 게임 복원 + 캐릭터 변경
  - index.html 원본(Phaser.js 기반) 복원
  - 클래스 이름 변경: 세척원→준설공, 정화원→세정공, 기술자→신호수
  - 클래스 아이콘 변경: 💧→🪣, 🔮→🔫, 🔧→🚩
  - 클래스 색상 변경: 시안/오렌지/레드 계열
  - 게임 로직(보너스, 무기, 패시브) 변경 없음
